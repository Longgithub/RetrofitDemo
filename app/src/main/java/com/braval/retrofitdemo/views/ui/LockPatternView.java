 ...
 */
package com.braval.retrofitdemo.views.ui;
...
public class LockPatternView extends View {
    ...
    public static final int ASPECT_SQUARE = 0; // View will be the minimum of width/height
    public static final int ASPECT_LOCK_WIDTH = 1; // Fixed width; height will be minimum of (w,h)
    public static final int ASPECT_LOCK_HEIGHT = 2; // Fixed height; width will be minimum of (w,h)
     ...
     */
    static final int MILLIS_PER_CIRCLE_ANIMATING = 700;
    /**
    ...
    final int mStrokeAlpha = 128;
    final Path mCurrentPath = new Path();
    final Rect mInvalidate = new Rect();
    final Rect mTmpInvalidateRect = new Rect();
    boolean mDrawingProfilingStarted = false;
    final Paint mPaint = new Paint();
    final Paint mPathPaint = new Paint();
    OnPatternListener mOnPatternListener;
    final ArrayList<Cell> mPattern = new ArrayList<Cell>(9);
    /**
     ...
     * progress animation.
     */
    final boolean[][] mPatternDrawLookup = new boolean[3][3];
    /**
    ...
    final float mDiameterFactor = 0.0f;
    final float mHitFactor = 0.6f;
    float mSquareWidth;
    float mSquareHeight;
    ...
    final boolean path;
    final float stokewidth;
    final float bitmapwidth;

    final int defaultColor = Color.parseColor("#dfdfdf");
    final int correctcolor = Color.parseColor("#29b0f4");
    final int wrongColor = Color.parseColor("#ff6666");
    ...
    public LockPatternView(Context context, AttributeSet attrs) {
        ...
        path = a.getBoolean(R.styleable.LockPatternView_path, true);
        stokewidth = a.getFloat(R.styleable.LockPatternView_stokewidth, 10);
        bitmapwidth = a.getDimension(R.styleable.LockPatternView_bitmapwidth, 20);
        ...
        mPathPaint.setStrokeWidth(stokewidth);
        mBitmapWidth = (int) bitmapwidth;
        mBitmapHeight = (int) bitmapwidth;
    }
    /**
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Modification the Stoke's weight
        final Path currentPath = mCurrentPath;
        currentPath.rewind();
        // draw the circles
        final int paddingTop = getPaddingTop();
        final int paddingLeft = getPaddingLeft();
        final int itemWidth = getWidth() / 3;
        final int itemHeight = getHeight() / 3;
        //Drawcircles
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                drawCircle(canvas, paddingLeft + itemWidth / 2 + j * itemHeight, paddingTop + itemHeight / 2 + i * itemWidth, drawLookup[i][j]);
            }
        }
        ...
        /*if (drawPath) {
            for (int i = 0; i < count - 1; i++) {
                ...
                float leftX = paddingLeft + paddingLeft + cell.column * squareWidth;
        ...
        if (drawPath) {
            if (mPatternDisplayMode == DisplayMode.Correct) {
                mPathPaint.setColor(correctcolor);
            } else if (mPatternDisplayMode == DisplayMode.Wrong) {
                mPathPaint.setColor(wrongColor);
            }
...
//            innerCircle = null;
        ...
        int radius = mBitmapHeight / 2;
        mPaint.setAntiAlias(true);
//        canvas.drawCircle(leftX + mBitmapHeight + radius / 2 + radius / 4, topY + mBitmapHeight + radius / 2 + radius / 4, radius, mPaint);
        canvas.drawCircle(leftX, topY, radius, mPaint);
...
//    }
    ...
    public static class Cell {
        ...
        static final Cell[][] sCells = new Cell[3][3];
...
// --Commented out by Inspection START (2015/5/26 17:29):
//        /**
...
//        }
// --Commented out by Inspection STOP (2015/5/26 17:29)
